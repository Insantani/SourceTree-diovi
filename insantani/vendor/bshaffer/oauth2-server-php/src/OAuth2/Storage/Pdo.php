...
//        return $user['password'] == $password;
    ...
    public function setUser($email, $password, $name = null)
    {
        ...
        if ($this->getUser($email)) {
            $stmt = $this->db->prepare($sql = sprintf('UPDATE %s SET password=:password, name=:name where email=:email', $this->config['user_table']));
        } else {
            $stmt = $this->db->prepare(sprintf('INSERT INTO %s (email, password, name) VALUES (:email, :password, :name)', $this->config['user_table']));
        }
